...
def RGB2Lab(Ima):
    ...
    ImaLab = reshape(vstack((L2.flatten(), a2.flatten(), b2.flatten())).T,ImaLab.shape)
...
def SampleColorNaming(s):
    ...
    Lab = RGB2Lab(double(reshape(s,(1,1,3))))
    L=Lab[:,:,0].flatten();
    a=Lab[:,:,1].flatten();
    b=Lab[:,:,2].flatten();
